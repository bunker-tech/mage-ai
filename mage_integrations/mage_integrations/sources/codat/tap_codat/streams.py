...
class CompanyInfo(Stream):
    def raw_fetch(self, ctx):
        return ctx.client.GET({"path": self.path.format(companyId=ctx.config.get('company_id'))}, self.tap_stream_id)

    def fetch_into_cache(self, ctx):
        resp = self.raw_fetch(ctx)
        resp['id'] = ctx.config.get('company_id')
        ctx.cache["companies"] = [resp]
...
all_streams = [
    ...
    PaginatedWithConnectionId("direct_costs",
        ...
        collection_key="results", 
        state_filter="modifiedDate"),
    PaginatedWithConnectionId("account_categories_v3",
        ["id", "companyId", "connectionId"],
        "data/companies/{companyId}/connections/{connectionId}/assess/accounts/categories/v3",
        collection_key="results",
    ),
